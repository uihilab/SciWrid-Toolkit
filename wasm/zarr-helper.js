 ...
 *
 *   WebParsersZarr.scan(uint8array)
 *     → list every Zarr array in a ZIP-of-zarr, with shape/dtype/chunks
 *
 *   WebParsersZarr.scanGetVarsJson(scanResult)
 *     → JSON string in the same shape wp_scan_get_vars_json emits
 *       (compatible with the existing test.html UI)
 *
 *   WebParsersZarr.normalize(scanResult, varIndex, wasm)
 ...
 *   for gzip / deflate-raw, and JS has small, fast ZIP parsers.  Doing
 *   this in WASM would mean shipping zstd + blosc + zip + json libraries
 *   for no practical gain — instead we hand the C engine a flat float
 *   array via wp_open_from_float_arrays once everything's decoded.
 *
 * Supported v2 features
 *   - dtypes: <f4 <f8 <i1 <i2 <i4 <u1 <u2 <u4 |i1 |u1
 *   - compressors: null (raw), gzip / zlib (DecompressionStream)
 *   - byte order: little-endian only (real-world default)
 *   - dimension_separator: '.' and '/'
 *   - fill_value: number, "NaN", "Infinity", "-Infinity"
 *
 * Not yet (clear extension points marked TODO):
 *   - zstd, blosc, lz4 (need codec libs)
 *   - filters
 *   - big-endian dtypes
 */
...
(function (global) {
  ...
  async function decompressChunk(bytes, compressor) {
    ...
    /* TODO codec hooks — wire in when libs are bundled
     *   id === 'zstd'  → zstd-codec / zstddec
     *   id === 'blosc' → blosc.js
     *   id === 'lz4'   → lz4js
     */
    throw new Error('Unsupported Zarr compressor: ' + id +
      '  (only null, gzip, zlib are built-in; zstd / blosc need a codec lib)');
  }
  ...
  /**
   * Emit the same JSON shape as wp_scan_get_vars_json so test.html can
   * render the variables table without special-casing zarr.
   */
  ...
  async function readArrayAsFloat32(scanResult, arrayInfo) {
    ...
    if (meta.filters)
      throw new Error('Zarr filters not supported yet');
