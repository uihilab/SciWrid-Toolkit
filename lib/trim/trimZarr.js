 ...
 * lib/trim/zarr.js
 *
 * Zarr v2 (zip-of-zarr) trim: zip-entry filtering with targeted coordinate
 * decoding.
 *
 * Strategy:
 ...
 *      to learn the chunk grid and named dimensions.
 *   3. Decide which chunks to keep for time and/or bbox slices. Data chunks
 *      are kept whole, so slice bounds widen to chunk boundaries and warnings
 *      describe the widened extent.
 *   4. Re-emit a new ZIP containing .zgroup, top-level .zattrs (if any), and
 *      each kept variable's metadata plus selected chunks. Entry method
 *      (stored=0 / deflate=8) is preserved per passthrough entry.
 *   5. Coordinate arrays sharing a sliced dimension are decoded and re-sliced
 *      to match (single-chunk, uncompressed coords only).
 *
 ...
 *
 * Out of scope (v1): partial-chunk extraction along sliced data axes (which
 * would require decode + re-encode), multi-chunk/compressed coord re-slicing,
 * curvilinear 2-D coords, and nested Zarr groups.
 */

import { TrimError } from './errors.js';
/* The ZIP container writer is shared with lib/encode/zarr.js. */
import { buildZip as writeZip } from '../encode/zip-writer.js';
import { VariableNotFoundError, UnsupportedFormatError } from '../errors.js';
...
function findEOCD(data) {
  ...
  throw new TrimError('Zarr trim: not a ZIP file (no EOCD found)');
}
...
function parseZipEntries(data) {
  ...
  for (let i = 0; i < nEntry && p < cdEnd; i++) {
    if (dv.getUint32(p, true) !== ZIP_CD_SIG)
      throw new TrimError(`Zarr trim: corrupt central directory at ${p}`);
    ...
    if (dv.getUint32(localHdr, true) !== 0x04034b50)
      throw new TrimError(
        `Zarr trim: corrupt local header for entry '${name}' at ${localHdr}`);
 ...
 * (e.g. a rewritten .zarray with the trimmed shape).
 * ----------------------------------------------------------------------- */
...
async function readEntryBytes(data, entry) {
  ...
  throw new TrimError(
    `Zarr trim: entry '${entry.name}' uses unsupported ZIP compression ` +
 ...
 * has an unsupported dtype — caller falls back to "no bbox trim possible".
 */
...
function computeAxisCuts(name, zarray, zattrs, ctx, warnings) {
  ...
  if (haveT) {
    ...
    if (t1 >= tLen)
      throw new TrimError(`Zarr trim: t1=${t1} >= time axis length (${tLen}) for variable '${name}'`);
  ...
  if (haveBbox) {
    ...
    if (latAxis < 0 || lonAxis < 0)
      throw new TrimError(`Zarr trim: variable '${name}' has no lat/lon axis in _ARRAY_DIMENSIONS — bbox cannot be applied`);
    ...
    if (!latIdx || !lonIdx)
      throw new TrimError(`Zarr trim: bbox does not intersect variable '${name}' lat/lon extent`);
...
export async function trim(byteSource, opts /*, ctx */) {
  ...
  /* Zarr v3 describes arrays with `zarr.json`, not `.zarray`, so groupEntries()
   * finds nothing and every requested variable would be reported "not present"
   * -- a false diagnosis that sends callers checking their own spelling.
   * Detect it and say what is actually true. */
  if (allVars.size === 0 && entries.some(e => e.name.endsWith('zarr.json')))
    throw new UnsupportedFormatError(
      'Zarr trim: this archive is Zarr v3 (zarr.json metadata); trim currently ' +
      'supports Zarr v2 (.zarray) only');
  ...
  if (missing.length === opts.variables.length)
    throw new VariableNotFoundError(
      `Zarr trim: none of the requested variables are present ` +
      `(requested: ${opts.variables.join(', ')}; ` +
      `available: ${[...allVars.keys()].join(', ')})`);
  if (missing.length > 0)
    throw new VariableNotFoundError(
      `Zarr trim: variable(s) not found: ${missing.join(', ')}`);
  ...
  if (haveBbox) {
    ...
    if (!bboxLatCoord || !bboxLonCoord)
      throw new TrimError(
        'Zarr trim: bbox requires single-chunk uncompressed 1-D lat/lon coord ' +
   ...
   * Without these sibling arrays, scan/extract can re-open the trimmed store
  ...
  for (const [name, v] of allVars.entries()) {
    ...
    // For coord arrays: their trim happens through their own iteration (each
