...
export async function trim(source, opts) {
  ...
  if (fmt == null) {
    /* Parquet's 'PAR1' magic is at byte 0, but detectFormat() confirms parquet
     * by reading the footer at EOF, which this 16-byte head cannot supply --
     * so it returns null. Parquet is genuinely out of scope for trim; name it
     * rather than claim we could not identify the file. */
    if (head.length >= 4 &&
        head[0] === 0x50 && head[1] === 0x41 && head[2] === 0x52 && head[3] === 0x31) {
      try { await byteSource.close?.(); } catch (_) {}
      throw new UnsupportedFormatError(
        'trim: parquet is not supported; supported formats are ' +
        'grib2, netcdf3, netcdf4, zarr, and tiff');
    }
    throw new UnsupportedFormatError(
