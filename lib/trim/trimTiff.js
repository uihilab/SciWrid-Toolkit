/*
 * lib/trim/trimTiff.js
 *
 * trim() implementation for TIFF / GeoTIFF — v2 scope: band selection.
 *
 *   trim(tiff, { variables: ['B04_red'] })
 ...
 *
 * Returns { bytes, format, warnings, stats } per the trim() contract.
 */
...
function dtypeOf(bps, sf) {
  ...
  throw new UnsupportedFormatError(`trim/tiff: unsupported sample (sampleFormat=${sf}, bps=${bps})`);
}
...
async function decodeBlock(rawBytes, compression) {
  ...
  throw new UnsupportedFormatError(`trim/tiff: compression ${compression} not supported`);
}
async function encodeBlock(rawBytes, compression) {
  ...
  throw new UnsupportedFormatError(`trim/tiff: cannot re-encode compression ${compression}`);
}
...
export async function trim(byteSource, opts /*, ctx */) {
  ...
  if (ifds.length > 1)
    throw new TrimError('trim/tiff: multi-IFD (COG with overviews) not supported in v2 — strip the overviews first');
  const ifd  = ifds[0];
  if (ifd.bigtiff)
    throw new TrimError('trim/tiff: BigTIFF trim not supported in v2');
  if (ifd.le === false)
    throw new TrimError('trim/tiff: big-endian TIFF trim not supported in v2');
  const t    = ifd.tags;

  const planar = tagVal(t, TAG.PlanarConfiguration, 1);
  if (planar !== 1 && planar !== 2)
    throw new TrimError(`trim/tiff: PlanarConfiguration=${planar} not supported`);
  ...
  if (!compression)
    throw new TrimError(`trim/tiff: compression code ${compId} not supported`);
  ...
  for (const want of opts.variables) {
    ...
    if (idx === -1)
      throw new VariableNotFoundError(`trim/tiff: unknown variable '${want}' (available: ${names.join(', ')})`);
    keep.push(idx);
  }

  // No-op fast path: keep every band AND no bbox → byte-copy.
  const noOp = !opts.bbox &&
               keep.length === samplesPerPixel &&
               keep.every((v, i) => v === i);
  if (noOp) {
    return {
      bytes: data,
  ...
  // ── Spatial bbox clipping (optional). Aligns to the block grid; partial
  //    blocks are widened to the next block boundary and a warning is added. ──
  const warnings = [];
  let newWidth  = width;
  let newHeight = height;
  let tilesAcross  = isTile ? Math.ceil(width  / blockW) : 1;
  let tilesDown    = isTile ? Math.ceil(height / blockH) : Math.ceil(height / blockH);
  let keepCols = { c0: 0, c1: tilesAcross };
  let keepRows = { r0: 0, r1: tilesDown };
  let newX0, newY0;
  const sx = tagVal(t, TAG.ModelPixelScale)?.[0];
  const sy = tagVal(t, TAG.ModelPixelScale)?.[1];
  const tiepoint = t.get(TAG.ModelTiepoint)?.values;
  const X0 = tiepoint?.[3];
  const Y0 = tiepoint?.[4];

  if (opts.bbox) {
    if (sx == null || sy == null || X0 == null || Y0 == null)
      throw new TrimError('trim/tiff: bbox trim requires ModelTiepoint + ModelPixelScale');
    const geo = parseGeoKeys(t);
    const [minLon, minLat, maxLon, maxLat] = opts.bbox;
    // Project the four bbox corners and take the AABB in native CRS.
    const corners = [
      latLonToNative({ lat: minLat, lon: minLon }, geo),
      latLonToNative({ lat: maxLat, lon: minLon }, geo),
      latLonToNative({ lat: minLat, lon: maxLon }, geo),
      latLonToNative({ lat: maxLat, lon: maxLon }, geo),
    ];
    const xs = corners.map(p => p.x);
    const ys = corners.map(p => p.y);
    // Pixel-window indices, clipped to the image.
    const px0 = Math.max(0,        Math.floor((Math.min(...xs) - X0) / sx));
    const px1 = Math.min(width,    Math.ceil ((Math.max(...xs) - X0) / sx));
    const py0 = Math.max(0,        Math.floor((Y0 - Math.max(...ys)) / sy));
    const py1 = Math.min(height,   Math.ceil ((Y0 - Math.min(...ys)) / sy));
    if (px1 <= px0 || py1 <= py0)
      throw new TrimError('trim/tiff: bbox does not intersect the image extent');

    // Snap the pixel window to block boundaries (widen outward).
    const c0 = Math.floor(px0 / blockW);
    const c1 = Math.ceil (px1 / blockW);
    const r0 = Math.floor(py0 / blockH);
    const r1 = Math.ceil (py1 / blockH);
    keepCols = { c0, c1 };
    keepRows = { r0, r1 };

    if (px0 !== c0 * blockW || px1 !== c1 * blockW || py0 !== r0 * blockH || py1 !== r1 * blockH) {
      warnings.push(
        `bbox snapped to ${isTile ? 'tile' : 'strip'} boundaries (output covers ` +
        `${c1 * blockW - c0 * blockW}×${r1 * blockH - r0 * blockH} pixels at native resolution; ` +
        `requested ${px1 - px0}×${py1 - py0})`);
    }

    newWidth  = (c1 - c0) * blockW;
    newHeight = (r1 - r0) * blockH;
    // Don't exceed the original image extent (last block may be partial).
    if (c1 === tilesAcross) newWidth  = width  - c0 * blockW;
    if (r1 === tilesDown)   newHeight = height - r0 * blockH;
    newX0 = X0 + c0 * blockW * sx;
    newY0 = Y0 - r0 * blockH * sy;
  }

  const newBlocks = [];

  // Helper: enumerate the (col, row) block indices that survive the bbox.
  function* keptBlockGrid() {
    for (let r = keepRows.r0; r < keepRows.r1; r++) {
      for (let c = keepCols.c0; c < keepCols.c1; c++) {
        yield { r, c };
      }
    }
  }
  ...
  if (planar === 2) {
    ...
    // For each kept band, copy the kept-bbox subset VERBATIM (no decode).
    const blocksPerPlane = offsetsArr.length / samplesPerPixel;
    if (!Number.isInteger(blocksPerPlane))
      throw new TrimError(`trim/tiff: planar=2 offsets length ${offsetsArr.length} is not a multiple of SPP ${samplesPerPixel}`);
    for (const bandIdx of keep) {
      for (const { r, c } of keptBlockGrid()) {
        const k = bandIdx * blocksPerPlane + r * tilesAcross + c;
  ...
  // ── Rebuild the IFD: rewrite per-band tags + GDAL_METADATA + bbox tags ─
  const newTags = [];
  ...
  for (const tg of newTags) {
    ...
    // bbox: update image extent + tiepoint origin.
    else if (opts.bbox && tg.tag === TAG.ImageWidth)  tg.values = [newWidth];
    else if (opts.bbox && tg.tag === TAG.ImageLength) tg.values = [newHeight];
    else if (opts.bbox && tg.tag === TAG.ModelTiepoint) {
      // ModelTiepoint = [pixI, pixJ, pixK, X, Y, Z]; only X/Y move.
      tg.values = [...tg.values];
      tg.values[3] = newX0;
      tg.values[4] = newY0;
    }
