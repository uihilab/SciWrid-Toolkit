 ...
 * lib/trim/grib2.js
 *
 * GRIB2 trim — message-stream byte-cut. No decode.
 *
 * Pipeline:
 *   1. Pull the full source into a Uint8Array (trim has to address every
...
function copyIntoWasm(wasm, data) {
  ...
  if (ptr === 0) throw new TrimError('GRIB2 trim: WASM out of memory');
...
/**
 * @param {object} byteSource          byte-source (lib/trim/byte-source.js)
...
export async function trim(byteSource, opts, ctx) {
  ...
  try {
    ...
    if (scanPtr === 0)
      throw new TrimError('GRIB2 trim: wp_scan failed (malformed file?)');

    const jsonPtr = wasm.ccall('wp_scan_messages_layout', 'number',
                               ['number'], [scanPtr]);
    if (jsonPtr === 0)
      throw new TrimError('GRIB2 trim: wp_scan_messages_layout returned NULL');
    ...
    return assembleTrim(data, messages, opts);
  } finally {
    if (scanPtr) {
...
/* ---------------------------------------------------------------------- */
/* Trim assembly: pure filter + concat over the parsed message index.     */
/* Pulled out for direct unit-testability without spinning up WASM.       */
/* ---------------------------------------------------------------------- */
export function assembleTrim(data, messages, opts) {
  ...
  if (missing.length === opts.variables.length)
    throw new VariableNotFoundError(
      `GRIB2 trim: none of the requested variables are present in the file ` +
      `(requested: ${opts.variables.join(', ')}; ` +
      `available: ${[...availableNames].join(', ')})`);
  if (missing.length > 0)
    throw new VariableNotFoundError(
      `GRIB2 trim: variable(s) not found in file: ${missing.join(', ')}`);
  ...
  for (const [name, arr] of byVar.entries()) {
    ...
    if (haveT) {
      if (t1 >= arr.length)
        throw new TrimError(
          `GRIB2 trim: t1=${t1} >= number of timesteps (${arr.length}) ` +
          `for variable '${name}'`);
