...
export async function grib2RangePointExtract(url, query, opts = {}) {
  return _fromReader(new GrbRangeReader(url, { fetchImpl: opts.fetchImpl }), query, opts);
}

/* Local seekable source (Node): extract from a filesystem path without loading
 * the whole file — fixes large local files the whole-file path OOMs. */
export async function grib2RangePointExtractFile(path, query, opts = {}) {
  const { GrbFileRangeReader } = await import('./grb-file-range-reader.js');
  const reader = new GrbFileRangeReader(path);
  try { return await _fromReader(reader, query, opts); }
  finally { reader.close(); }
}

async function _fromReader(reader, query, opts = {}) {
  try {
    ...
    // Stream each selected message: decode → pull the known cell → discard.
    // Single-message buffers have the target variable at index 0; the lean
    // wp_scan_decode_cell reuses the parsed grid + (iy,ix) — no coord rebuild,
    // no dataset, no JSON per message.
    const timeseries = [];
    for (const m of sel) {
      const scan = scanOne(wasm, await reader.read(m.offset, m.length));
      if (!scan) continue;
      const v = wasm.ccall('wp_scan_decode_cell', 'number',
        ['number', 'number', 'number', 'number'], [scan, 0, iy, ix]);
      timeseries.push({ time: isoUtc(m.time), value: Number.isNaN(v) ? null : v });
      wasm.ccall('wp_scan_free', null, ['number'], [scan]);
    }

    return { variable, location: { lat, lon }, timeseries, _stats: reader.stats() };
  } catch (e) {
    /* Returning null is right -- the caller must fall back -- but throwing the
     * reason away is not. The fallback produces a correct answer, so a lost
     * reason is undetectable downstream. Push it instead of raising: callers
     * outside lib/ (.testkit/perf/bench-grib2-*.mjs) expect null, not a throw. */
    if (opts && Array.isArray(opts.diag))
      opts.diag.push({ path: 'grib2-range', reason: String((e && e.message) || e) });
    return null;
  }
}
