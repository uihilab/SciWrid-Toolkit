...
function _parseFront(File, ab, variable) {
  ...
  if (rank >= 3 && timeName && byName[timeName]) {
     ...
     *
     * When it throws we now record the reason and leave `times` null, so the
     * caller emits time: null. The previous fallback stringified the raw
     * values, producing "391440" where the whole-file path produces
     * "2023-08-28T00:00:00Z" -- indistinguishable from a real timestamp to
     * anything downstream.
     *
     * KNOWN LIMITATION: jsfive 0.4 returns {} for EVERY dataset's attrs on
     * files written by netcdf 4.9.3 / HDF5 1.14.6, so `units` is undefined and
     * decodeTimes always throws for them. This is NOT front-buffer truncation:
     * it reproduces with the entire file in the buffer, while root attrs parse
     * fine. Fixing it means decoding HDF5 attribute messages ourselves; that is
     * its own piece of work. Tracked by the KNOWN entries in
     * .testkit/test-range-parity.js. */
    try {
