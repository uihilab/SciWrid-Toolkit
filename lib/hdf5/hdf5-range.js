...
async function _parseMetaAndChunks(reader, File, size, variable, fetchImpl) {
...
export async function hdf5RangePointExtract(url, query, opts = {}) {
  ...
  const parsed = await _parseMetaAndChunks(reader, File, size, query.variable, fetchImpl);
...
function _parseFront(File, ab, variable) {
  ...
  if (rank >= 3 && timeName && byName[timeName]) {
     ...
     * then omitted entirely, so the `time` key silently vanished from the
     * range result while the whole-file result carried it.
     *
     * KNOWN LIMITATION, not fixed here: for some files jsfive exposes NO
     * attributes for the time dataset off the front buffer (`attrs` comes back
     * as {}), so `units` is undefined, decodeTimes throws, and the fallback
     * below emits raw numbers ("391440") where the whole-file path emits
     * "2023-08-28T00:00:00Z". Reading those attributes needs the object-header
     * pointer-following described at the top of this file. Covered by the
     * failing `time values match` case in .testkit/test-range-parity.js. */
    try {
  ...
  const vattrs = ds.attrs || {};
  return {
    varName, shape, di, filters, chunkShape, chunkAddress: dob._chunk_address, rank,
    latAxis: rank - 2, lonAxis: rank - 1, lats, lonsRaw, times, nt,
    units: vattrs.units || '',
