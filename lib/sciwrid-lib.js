...
export class SciWridToolkit {
  ...
  _scanNetCDF3(data) {
    ...
    /* NetCDF3's compact scan JSON contains data variables but not coordinate
     * values. Read the small time coordinate directly from the full header
     * layout while the source bytes are available; never decode the data cube. */
    try { this._nc3AttachTimes(data); }
    catch (_) { /* Time metadata is best-effort, matching the other formats. */ }
  ...
  /* Decode a CF time coordinate from a NetCDF3 header layout and attach it to
   * every scanned data variable that uses the same dimension. Record-coordinate
   * values use the file's interleaved record stride; fixed coordinates are
   * contiguous. */
  _nc3AttachTimes(data) {
    const wasm = this.wasm;
    if (!wasm || !this.scanPtr || typeof wasm._wp_nc3_full_layout !== 'function') return;
    ...
    if (!layout || !Array.isArray(layout.vars) || !Array.isArray(layout.dims)) return;

    const attrText = (variable, name) => {
      const attr = (variable.atts || []).find((item) => item.name === name);
      if (!attr || typeof attr.value_b64 !== 'string') return '';
      try {
        const text = typeof atob === 'function'
          ? atob(attr.value_b64)
          : Buffer.from(attr.value_b64, 'base64').toString('latin1');
        return text.replace(/\0+$/, '');
      } catch (_) { return ''; }
    };

    const timeVar = layout.vars.find((variable) => {
      const name = String(variable.name || '').toLowerCase();
      const axis = attrText(variable, 'axis').toUpperCase();
      const units = attrText(variable, 'units').toLowerCase();
      return variable.ndims === 1 &&
        (name === 'time' || axis === 'T' || /^[a-z]+\s+since\s+/.test(units));
    });
    if (!timeVar || !Array.isArray(timeVar.dim_indices) || timeVar.dim_indices.length !== 1) return;

    const count = Number(timeVar.shape?.[0]);
    const elementSize = Number(timeVar.type_size);
    const begin = Number(timeVar.begin);
    if (!(count > 0) || !(elementSize > 0) || !Number.isFinite(begin)) return;

    const recordStride = layout.vars.reduce(
      (sum, variable) => sum + (variable.is_record ? Number(variable.vsize) || 0 : 0), 0);
    const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
    const readValue = (offset) => {
      switch (timeVar.type) {
        case 1: return view.getInt8(offset);
        case 3: return view.getInt16(offset, false);
        case 4: return view.getInt32(offset, false);
        case 5: return view.getFloat32(offset, false);
        case 6: return view.getFloat64(offset, false);
        default: return NaN;
      }
    };
    const values = new Float64Array(count);
    for (let i = 0; i < count; i++) {
      const offset = timeVar.is_record ? begin + i * recordStride : begin + i * elementSize;
      if (offset < 0 || offset + elementSize > data.byteLength) return;
      values[i] = readValue(offset);
      if (!Number.isFinite(values[i])) return;
    }

    const units = attrText(timeVar, 'units');
    if (!units) return;
    const calendar = attrText(timeVar, 'calendar') || 'standard';
    const decoded = decodeTimes(values, units, calendar);
    const timeDim = timeVar.dim_indices[0];
    for (const variable of this.vars) {
      const full = layout.vars.find((item) => item.name === variable.name);
      if (full?.dim_indices?.includes(timeDim)) variable.times = decoded;
    }
  }
  ...
  /* Lazy-load h5wasm only when a NetCDF4 file is first opened. Resolution and
   * caching live in lib/hdf5/load-h5wasm.js, shared with the NetCDF4 writer:
   * h5wasm's Emscripten FS is process-global, so the realm must hold exactly
   * one module instance.
   *
   * Override:
   *   new SciWridToolkit({ h5wasmUrl: '<your-mirror-url>' })
   */
  async _getH5wasm() {
    if (this._h5wasmModule) return this._h5wasmModule;
    const { loadH5wasm } = await import('./hdf5/load-h5wasm.js');
    this._h5wasmModule = await loadH5wasm(this.config?.h5wasmUrl);
