...
import { resolveByteSource }    from './trim/byte-source.js';
...
export async function extract(source, { variable, lat, lon } = {}) {
  ...
  const planarConfig = tagVal(t, TAG.PlanarConfiguration, 1);
  // For separate planes (planar=2), each block holds pixels of ONE band only.
  // Address it as if SPP=1, bandIndex=0; the band is selected via the strip
  // group inside locatePixel.
  const sppBlock  = planarConfig === 2 ? 1 : samplesPerPixel;
  const bandBlock = planarConfig === 2 ? 0 : bandIdx;
  const blk = locatePixel(t, row, col, { planarConfig, bandIndex: bandIdx, samplesPerPixel });
    ...
    for (let ox = 0; ox < outW; ox++) {
      ...
      if (!decoded) {
        const blk = locatePixel(t, by * blockH, bx * blockW,
          { planarConfig, bandIndex: bandIdx, samplesPerPixel });
        const raw = (blk.offset + blk.length <= buf.length)
          ? buf.subarray(blk.offset, blk.offset + blk.length)
          : await bs.read(blk.offset, blk.length);
        decoded = await decodeBlock(raw, compression, { samplesPerPixel: sppBlock });
        unpredict(decoded, { predictor, width: blk.blockWidth, height: blk.blockHeight,
                             samplesPerPixel: sppBlock, dtype, le });
        cache.set(key, decoded);
      }
      const v = sampleValue(decoded, {
        rowInBlock: r - by * blockH, colInBlock: c - bx * blockW,
        blockWidth: blockW, samplesPerPixel: sppBlock, bandIndex: bandBlock, dtype, le,
      });
      out[oy * outW + ox] = v;
    }
  }
