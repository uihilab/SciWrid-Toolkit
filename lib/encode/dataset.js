...
/* Fold either point-series shape into one array of {time, value}. */
export function normaliseSamples(result) {
  if (Array.isArray(result.timeseries)) return result.timeseries;
  const times = result.times;
  const values = result.values;
  if (!Array.isArray(times) || values == null) return [];
  /* values may be a typed array, a plain array, or an index-keyed object */
  const at = (i) => (ArrayBuffer.isView(values) || Array.isArray(values))
    ? values[i]
    : values[String(i)];
  return times.map((time, i) => ({ time, value: at(i) }));
}
...
export function seriesToDataset(result) {
  ...
  /* extract() does not return one shape for every format: the WASM-backed
   * readers give timeseries:[{time,value}], while the Parquet and TIFF paths
   * give parallel times[] and values[]. Both are point series, so both are
   * accepted here rather than making the caller care which reader ran. */
  const ts = normaliseSamples(result);
  if (!ts.length)
    throw new UnsupportedExportError(
      'encodeSeries: result has no time series. Use json or csv.');
