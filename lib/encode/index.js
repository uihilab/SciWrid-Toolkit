...
import { encodeDatasetZarr } from './zarr.js';
import { encodeDatasetGRIB2 } from './grib2.js';
/* netcdf4 pulls in h5wasm, so it is loaded lazily — a caller who never asks
 * for it must not pay the bundle or the wasm init. */
...
export const EXPORT_FORMATS = Object.freeze([
  ...
  { id: 'zarr',    label: 'Zarr v2 (zip)',     ext: 'zip',  mime: 'application/zip',
    grid: true, series: true,  binary: true },
  { id: 'netcdf4', label: 'NetCDF-4 / HDF5',    ext: 'nc',   mime: 'application/x-netcdf',
    grid: true, series: true,  binary: true },
  { id: 'grib2',   label: 'GRIB2',              ext: 'grb2', mime: 'application/x-grib',
    grid: true, series: false, binary: true },
...
const ALIASES = { tif: 'geotiff', tiff: 'geotiff',
                  ...
                  nc4: 'netcdf4', hdf5: 'netcdf4', h5: 'netcdf4',
                  grib: 'grib2', grb2: 'grib2', grb: 'grib2',
                  pq: 'parquet', parq: 'parquet' };
...
export async function encodeGrid(grid, format = 'json', opts = {}) {
  ...
  if (row.id === 'zarr')    return encodeDatasetZarr(dataset);
  if (row.id === 'netcdf4') {
    const { encodeDatasetNetCDF4 } = await import('./netcdf4.js');
    return encodeDatasetNetCDF4(dataset, opts);
  }
  if (row.id === 'grib2')   return encodeDatasetGRIB2(dataset, opts);
  if (row.id === 'parquet') {
    const { encodeDatasetParquet } = await import('./parquet.js');
    return encodeDatasetParquet(dataset, opts);
  }
