...
// Turn a date / dateRange selection into an absolute [minEpochSeconds, maxEpochSeconds]
// window so scan() can prune row groups on the time column. Index-based selection (t)
// can't prune without the axis, so it is left alone.
function deriveTimeRange(opts) {
  if (Array.isArray(opts.dateRange) && opts.dateRange.length === 2) {
    const a = Date.parse(opts.dateRange[0]);
    const b = Date.parse(opts.dateRange[1]);
    if (Number.isFinite(a) && Number.isFinite(b)) {
      return [Math.floor(Math.min(a, b) / 1000), Math.floor(Math.max(a, b) / 1000)];
    }
  }
  if (opts.date != null) {
    const d = Date.parse(opts.date);
    if (Number.isFinite(d)) { const s = Math.floor(d / 1000); return [s, s + 86399]; }
  }
  return null;
}

function withTimeRange(opts) {
  if (opts._timeRange) return opts;
  const tr = deriveTimeRange(opts);
  return tr ? { ...opts, _timeRange: tr } : opts;
}
...
export async function extract(source, opts = {}) {
  const scanResult = await scan(source, withTimeRange(opts));
  try {
