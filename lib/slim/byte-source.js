/*
 * lib/slim/byte-source.js
 *
 * Dual-runtime random-access byte source for the slim pipeline. The slim
...
export function fromUint8Array(buf) {
  if (!(buf instanceof Uint8Array))
    throw new SlimError('fromUint8Array: expected Uint8Array');
  return {
...
export function fromBlob(blob) {
  if (typeof Blob === 'undefined' || !(blob instanceof Blob))
    throw new SlimError('fromBlob: expected a Blob');
  return {
...
export async function fromHttpRange(url) {
  // Probe via HEAD; fall back to a tiny range GET if HEAD is forbidden.
  let total, acceptsRange;
  try {
    const head = await fetch(url, { method: 'HEAD' });
    if (!head.ok) throw new Error(`HEAD ${url}: ${head.status}`);
    total = Number(head.headers.get('content-length'));
    acceptsRange = (head.headers.get('accept-ranges') || '').toLowerCase().includes('bytes');
  } catch {
    const probe = await fetch(url, { headers: { Range: 'bytes=0-0' } });
    if (!probe.ok && probe.status !== 206)
      throw new SourceError(`byte-source: cannot probe ${url}: ${probe.status}`);
    const cr = probe.headers.get('content-range');
    total = cr ? Number(cr.split('/').pop()) : Number(probe.headers.get('content-length'));
    acceptsRange = probe.status === 206;
  }

  if (!Number.isFinite(total)) throw new SourceError(`byte-source: no Content-Length for ${url}`);
  ...
  return {
    ...
    async read(offset, length) {
      if (offset < 0 || length < 0 || offset + length > total)
        throw new SlimError(`byte-source: read [${offset},${offset+length}) out of range for size ${total}`);
      if (!acceptsRange) {
...
export async function resolveByteSource(source) {
  ...
  if (str == null)
    throw new SourceError(
      'slim source: expected Uint8Array, ArrayBuffer, Blob, URL, or string ' +
