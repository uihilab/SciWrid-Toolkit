...
export async function resolveByteSource(source) {
  ...
  if (/^https?:\/\//i.test(str)) {
    /* v1: fetch fully into memory. HTTP Range support tracked as a follow-up. */
    const resp = await fetch(str);
    if (!resp.ok)
      throw new SourceError(`slim: failed to fetch ${str}: ${resp.status} ${resp.statusText}`);
    return fromUint8Array(new Uint8Array(await resp.arrayBuffer()));
  }
