...
async function decodeSingleChunkCoord(data, vars, name) {
  ...
  if (!v) return null;
  const zarrayBytes = readEntryUtf8(data, v.zarray);
  ...
  // Read raw chunk bytes (stored, uncompressed) — pull from the source buffer
  // using the local-file-header offset already computed by parseZipEntries.
  const bytes = data.subarray(chunkEntry.dataStart, chunkEntry.dataStart + chunkEntry.compSize);
  const dtype = String(meta.dtype || '');
  const N = meta.shape[0];
  let arr;
  if (/^[<|]f4$/.test(dtype)) arr = new Float32Array(bytes.buffer, bytes.byteOffset, N);
  else if (/^[<|]f8$/.test(dtype)) arr = new Float64Array(bytes.buffer, bytes.byteOffset, N);
  else if (/^[<|][iu][12]$/.test(dtype)) {
    const Ctor = /^[<|]i2$/.test(dtype) ? Int16Array : /^[<|]u2$/.test(dtype) ? Uint16Array
               : /^[<|]i1$/.test(dtype) ? Int8Array : Uint8Array;
    arr = new Ctor(bytes.buffer, bytes.byteOffset, N);
  }
  else return null;
