/*
 * zarr-helper.js  --  Browser-side Zarr v2 + v3 reader for SciWrid Toolkit
 *
...
import { decodeTimes, normalizeCalendar } from './time-decoder.js';
...
/* Mirrors how _normalizeNetCDF4 in sciwrid-lib.js works.               */
/* ====================================================================== */
 ...
 *   wasm:       the loaded SciWridToolkit WASM module
 *
 ...
 */
/* CF packing. A Zarr store may pack floats into integers via scale_factor /
 * add_offset — AORC stores precipitation as int16 * 0.1. The NetCDF4 path has
 * always honoured these (sciwrid-lib.js:634-635); without the same handling
 * here a packed store reads back 1/scale too large.
 *
 * Deliberately narrow: this touches ONLY scale_factor / add_offset. Fill and
 * missing values are left exactly as they were, so unpacked stores — every
 * existing fixture — take the early return and are bit-identical to before.
 * NaN propagates through the arithmetic on its own. */
function applyCFPacking(data, attrs) {
  const at = attrs || {};
  if (at.scale_factor == null && at.add_offset == null) return data;
  const scale  = at.scale_factor != null ? Number(at.scale_factor) : 1;
  const offset = at.add_offset   != null ? Number(at.add_offset)   : 0;
  if (!Number.isFinite(scale) || !Number.isFinite(offset)) return data;
...
async function normalize(scanResult, varIndex, wasm, opts = {}) {
  ...
  /* Unpack after BOTH read paths (windowed and whole-array) so a packed store
   * behaves identically whichever branch ran. Coordinate arrays are read
   * separately below and are intentionally not unpacked. */
  data = applyCFPacking(data, a.attrs);
  ...
  /* The C engine reads this buffer as raw unix epoch SECONDS. A CF time axis
   * is "<unit> since <epoch>", so the raw coordinate value only happens to be
   * epoch seconds for `seconds since 1970-01-01`. Decode CF units here exactly
   * as the scan path does above — otherwise `days since 2001-01-01` value 0
   * renders as 1970-01-01 while scan() correctly reports 2001-01-01.
   *
   * Read at Float64: real epoch seconds (~1e9) exceed Float32's 2^24 exact
   * integer range, so a Float32 round-trip would silently corrupt them. */
  times_s = new Float64Array(nt);
  if (refs && refs.timeRef) {
    let rawFull;
    try {
      rawFull = await readArrayAsFloat64(scanResult, refs.timeRef);
    } catch (_) {
      rawFull = await readCoordOrIndex(refs.timeRef, shape[0], 'time');
    }

    const tAttrs   = (refs.timeRef && refs.timeRef.attrs) || {};
    const calendar = tAttrs.calendar || 'standard';
    let decodedSecs = null;

    /* Only the proleptic-Gregorian ("standard") calendar maps onto a real
     * instant. noleap / 360_day yield ISO strings such as 2001-02-30 that are
     * not valid Dates, so those keep the raw axis and record a warning. */
    if (tAttrs.units && normalizeCalendar(calendar) === 'standard') {
      try {
        const decoded = decodeTimes(Array.from(rawFull), tAttrs.units, calendar);
        const secs = decoded.values.map((iso) => Date.parse(iso) / 1000);
        if (secs.every(Number.isFinite)) decodedSecs = secs;
      } catch (e) {
        (scanResult._coordWarnings || (scanResult._coordWarnings = [])).push(
          `Zarr: could not decode CF time units "${tAttrs.units}" (${e.message}); ` +
          `using raw coordinate values.`);
      }
    } else if (tAttrs.units) {
      (scanResult._coordWarnings || (scanResult._coordWarnings = [])).push(
        `Zarr: calendar "${calendar}" has no unique mapping to an absolute ` +
        `instant; time values are reported on the raw axis.`);
    }

    for (let t = 0; t < nt; t++) {
      times_s[t] = decodedSecs ? decodedSecs[winStart + t] : rawFull[winStart + t];
    }
  } else {
...
/* ====================================================================== */
/* geoBbox — derive [minLon, minLat, maxLon, maxLat] from coord arrays.    */
/* ====================================================================== */

/**
 * Read the lat/lon coordinate arrays for the first geo-locatable variable
 * and return its geographic extent as [minLon, minLat, maxLon, maxLat].
 *
 * Mirrors the NetCDF4 `_nc4CoordBbox` path: a real bbox lets the map place a
 * regional Zarr store on its actual footprint instead of stretching it across
 * an assumed global box. Uses min/max (not first/last) so the result is
 * correct regardless of ascending vs descending coordinate order.
 *
 * Returns null when no variable resolves real lat/lon coords (synthetic axes)
 * or the coord arrays have no finite values — callers then fall back to the
 * assumed-global box exactly as before.
 */
async function geoBbox(scanResult) {
  const arrays = (scanResult && scanResult.arrays) || [];
  for (const a of arrays) {
    const shape = (a.meta && a.meta.shape) || [];
    if (shape.length < 2) continue;                 // need a 2-D+ data var

    const refs = resolveCoordRefs(scanResult, a);
    if (!refs.latRef || !refs.lonRef) continue;     // synthetic axes — skip
    if (refs.source === 'synthetic') continue;

    const range = async (ref) => {
      try {
        const vals = await readArrayAsFloat32(scanResult, ref);
    ...
    const lat = await range(refs.latRef);
    const lon = await range(refs.lonRef);
    if (lat && lon) return [lon[0], lat[0], lon[1], lat[1]];
  }
  return null;
}
...
export default {
  detectFormat, scan, scanGetVarsJson, normalize, geoBbox, scanFree, ZipChunkSource,
