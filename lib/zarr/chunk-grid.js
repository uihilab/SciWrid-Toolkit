...
import { ShardReader, innerToShard, innerLinearIndex } from './sharding.js';
...
async function readShardedArray(scanResult, arrayInfo, Ctor) {
  ...
  const dst = new Ctor(total);
  if (Number.isNaN(fill)) dst.fill(NaN);
  else if (Number.isFinite(fill)) dst.fill(fill);

  const grid = chunkGridDims(shape, inner);
  const reader = new ShardReader(meta);
  const shardKeyCache = new Map();
  const chunkLen = inner.reduce((a, b) => a * b, 1);

  for (const idx of chunkIndices(grid)) {
    const { shardIdx, innerLocalIdx } = innerToShard(meta, idx);
    const shardKey = chunkKey(meta, shardIdx);
    let rawShard = shardKeyCache.get(shardKey);
    if (rawShard === undefined) {
      rawShard = await scanResult.source.getChunkBytes(arrayInfo.name, shardKey);
      shardKeyCache.set(shardKey, rawShard);
    }
    if (!rawShard) continue;
    const lin = innerLinearIndex(meta, innerLocalIdx);
    const decoded = await reader.innerChunkBytes(scanResult.source, arrayInfo.name, shardKey, rawShard, lin);
    if (!decoded) continue;
    const typed = dt.view(decoded, 0, chunkLen);
    placeChunk(dst, shape, typed, inner, idx, fill);
  }
  return dst;
}
...
export async function readArrayAsFloat64(scanResult, arrayInfo) {
  const meta   = arrayInfo.meta;
  if (meta._sharding) return readShardedArray(scanResult, arrayInfo, Float64Array);
  const shape  = meta.shape;
...
export async function readArrayAsFloat32(scanResult, arrayInfo) {
  const meta   = arrayInfo.meta;
  if (meta._sharding) return readShardedArray(scanResult, arrayInfo, Float32Array);
  const shape  = meta.shape;
  const chunks = meta.chunks;
  const sep    = meta.dimension_separator || '.';
...
export async function readArrayWindowed(scanResult, arrayInfo, c0, c1) {
  ...
  if (meta._sharding) {
    return {
      data: await readShardedArray(scanResult, arrayInfo, Float32Array),
      winStart: 0,
      winLen: arrayInfo.meta.shape[0],
    };
  }
  const shape = meta.shape;
  const chunks = meta.chunks;
