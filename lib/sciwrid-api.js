 ...
 *
 * A URL is tried first on a range-native fast path (GRIB2, NetCDF4/HDF5, TIFF,
 * Parquet), which fetches only the bytes a query needs. Whatever that path
 * cannot handle falls back to downloading the whole file -- reusing the front
 * region the attempt already read, so declining costs no extra transfer.
 *
 * Errors: typed subclasses of SciWridError so callers can `instanceof`-check.
 */

import { SciWridToolkit } from './sciwrid-lib.js';
import { createCarry, completeFromCarry } from './sources/prefetch-carry.js';
...
/* The encoder layer. Every output format other than the two render targets
 * (imagedata, png) is resolved and written here, so extractOutput and
 * extractGridOutput stay thin. */
import { encodeGrid, encodeSeries, EXPORT_FORMATS, resolveFormat,
         UnsupportedExportError } from './encode/index.js';
 ...
 * ======================================================================= */
async function resolveSource(source, carry) {
  ...
  /* A range fast path may already have read the front of this exact URL and
   * then declined. Splicing a tail onto those bytes is what keeps a declined
   * attempt from costing the caller the file twice; see
   * lib/sources/prefetch-carry.js. Returns null for anything it is not certain
   * about, and then this falls through to the ordinary whole-file fetch. */
  if (carry && carry.bytes) {
    const whole = await completeFromCarry(carry, url);
 ...
 * ======================================================================= */
/* `carry` is a separate parameter rather than a field on `opts` because opts is
 * the caller's own options object, and it gets spread into lib.extract(). A
 * private plumbing field riding along in there would reach the class API. */
async function withInstance(source, opts, fn, carry) {
  const lib = new SciWridToolkit({
    wasmFactory: opts?.wasmFactory,
    h5wasmUrl:   opts?.h5wasmUrl,
  });
  try {
    const data = await resolveSource(source, carry);
...
async function tryHdf5RangePoint(source, options, diag, carry) {
  ...
  try {
    ...
    const out = await hdf5RangePointExtract(url,
      ...
      { fetchImpl: options.fetchImpl, diag, carry, forceRange: options.forceRange === true });
...
export async function extract(source, options = {}) {
  ...
  const diag = [];
  /* One carry per call. If the range path reads the front of the file and then
   * declines, those bytes go here and the whole-file fallback below splices
   * onto them instead of restarting at zero -- a decline costs one file total,
   * not one file plus the attempt. */
  const carry = createCarry();
  const fast = await tryHdf5RangePoint(source, options, diag, carry);
  if (fast) return fast;
...
export async function extractOutput(source, options = {}, format = 'json') {
  ...
  if (row.id !== 'json' && row.id !== 'csv') {
    /* Every other target goes through the encoder layer, which needs the
     * result object rather than the class's pre-serialised string. */
    const result = await extract(source, options);
    return encodeSeries(result, row.id, options);
  }
  /* json and csv keep their original path — the class produces them, and its
   * output must stay byte-identical. Formats the class cannot open at all
   * (Parquet, TIFF) never reached this code before and threw SourceError; they
   * now fall through to the encoder layer instead, which is strictly more than
   * they did, and changes nothing for the sources that already worked. */
  const dated = await resolveDateOptions(source, options, 'range');
  try {
    return await withInstance(source, dated, async (lib) => {
      let result;
      try {
        result = await lib.extract({ ...dated, type: row.id === 'csv' ? 'csv' : 'json' });
      } catch (e) {
        if (/not found/i.test(e.message) || /not supported/i.test(e.message))
          throw new VariableNotFoundError(e.message);
        throw new ExtractError(e.message);
      }
      /* The class honours type:'csv' only on the paths that implement it; the
       * Parquet path ignores it and hands back the result object. Rather than
       * return that to a caller who asked for CSV, fall through to the encoder
       * layer. Sources that already produced a CSV string are untouched. */
      if (row.id === 'csv' && typeof result !== 'string')
        return encodeSeries(result, 'csv', dated);
      return row.id === 'csv' ? result : JSON.stringify(result, null, 2);
    });
  } catch (e) {
...
async function tryHdf5RangeGrid(source, options, diag, carry) {
...
export async function extractGrid(source, options = {}) {
  const diag = [];
  const carry = createCarry();          // see extract() — same decline economics
  const fastGrid = await tryHdf5RangeGrid(source, options, diag, carry);
...
export async function extractGridOutput(source, options = {}, format = 'json') {
  ...
  /* imagedata and png are render targets, not file formats, so they stay out
   * of EXPORT_FORMATS and keep their existing path. */
  if (fmt === 'imagedata') return gridToImageData(await extractGrid(source, options), options);
  if (fmt === 'png')       return await gridToPNG(await extractGrid(source, options), options);
  const row = resolveFormat(fmt);
  const grid = await extractGrid(source, options);
  return encodeGrid(grid, row.id, options);
}
...
/* The encoder layer's public surface, re-exported so consumers who already
 * hold a grid can encode it without extracting again. */
export { encodeGrid, encodeSeries, EXPORT_FORMATS, UnsupportedExportError };

export default {
  detectFormat, scan, extract, extractOutput, extractGrid, extractGridOutput,
  gridToJSON, gridToGeoTIFF, gridToImageData, gridToPNG,
  encodeGrid, encodeSeries, EXPORT_FORMATS,
  RAMPS, resolveRamp, sampleRamp, trim,
  SciWridError, UnsupportedFormatError, VariableNotFoundError, SourceError, ExtractError,
  TrimError, UnsupportedCRSError, UnsupportedExportError,
};
