...
async function tryHdf5RangePoint(source, options, diag) {
  ...
  try {
    ...
    if (!out) diag.push({ path: 'hdf5-range', reason: 'range extractor returned no result' });
    return out;
  } catch (e) {
...
async function tryGrib2RangePoint(source, options, diag) {
  ...
  try {
    ...
    if (typeof url === 'string' && /^https?:\/\//i.test(url)) {
      ...
      const out = await grib2RangePointExtract(url, query, { fetchImpl: options.fetchImpl });
      if (!out) diag.push({ path: 'grib2-range', reason: 'range extractor returned no result' });
      return out;
    }
    ...
    if (localPath) {
      ...
      const out = await grib2RangePointExtractFile(localPath, query, {});
      if (!out) diag.push({ path: 'grib2-range', reason: 'range extractor returned no result' });
      return out;
    }
    return null;
...
async function tryHdf5RangeGrid(source, options, diag) {
  ...
  try {
    ...
    if (!out) diag.push({ path: 'hdf5-range-grid', reason: 'range extractor returned no result' });
    return out;
  } catch (e) {
