...
async function tryGrib2RangePoint(source, options) {
  try {
    if (options.variable == null || options.lat == null || options.lon == null) return null;
    ...
    if (typeof url === 'string' && /^https?:\/\//i.test(url)) {
      ...
      return await grib2RangePointExtract(url, query, { fetchImpl: options.fetchImpl });
    }
    ...
    if (localPath) {
      ...
      return await grib2RangePointExtractFile(localPath, query, {});
    }
    return null;
  } catch (_) { return null; }
}
...
export async function extract(source, options = {}) {
  ...
  if (fast) return fast;
  const fastGrib = await tryGrib2RangePoint(source, options);
  ...
  if (fmt === 'parquet') {
    ...
    return parquetExtract(source, options);
  }
  if (fmt === 'tiff') {
